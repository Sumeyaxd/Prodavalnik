...
import com.example.demo.model.dto.OrderOfferDetailsDTO;
...
import com.example.demo.service.events.MakeOrder;
...
class OrderServiceImplTest {
    ...
    public void testRemoveFromCart_Successful() {
        ...
        when(mockOfferService.findOfferById(dishId)).thenReturn(Optional.of(new Offer()));

        boolean result = orderServiceToTest.removeFromCart(dishId);

        assertTrue(result);
        assertFalse(orderServiceToTest.getAllOffersInCart()
                .getOffersToOrder().stream()
